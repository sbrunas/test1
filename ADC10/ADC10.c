...
//ch2 memory block-----------------------------------------------------------------------------------------
    ...
    if (!bcm2835_init())
    	return 1;	
//ch3 memory block-----------------------------------------------------------------------------------------
  	ch3 = malloc(sizeof(int32_t) * datacount); /* allocate memory for datacount int's */
 	if (!ch3) { /* If data == 0 after the call to malloc, allocation failed for some reason */
    	perror("Error allocating memory for channel 3");
    	abort();
	}
  	/* ch points to a valid block of memory.
     clearing block. */
  	memset(ch3, 0, sizeof(int32_t)*datacount);

    if (!bcm2835_init())
    	return 1;
//ch4 memory block-----------------------------------------------------------------------------------------
  	ch4 = malloc(sizeof(int32_t) * datacount); /* allocate memory for datacount int's */
 	if (!ch4) { /* If data == 0 after the call to malloc, allocation failed for some reason */
    	perror("Error allocating memory for channel 4");
    	abort();
	}
  	/* ch points to a valid block of memory.
     clearing block. */
  	memset(ch4, 0, sizeof(int32_t)*datacount);

    if (!bcm2835_init())
    	return 1;
//ch5 memory block-----------------------------------------------------------------------------------------
  	ch5 = malloc(sizeof(int32_t) * datacount); /* allocate memory for datacount int's */
 	if (!ch5) { /* If data == 0 after the call to malloc, allocation failed for some reason */
    	perror("Error allocating memory for channel 5");
    	abort();
	}
  	/* ch points to a valid block of memory.
     clearing block. */
  	memset(ch5, 0, sizeof(int32_t)*datacount);

    if (!bcm2835_init())
    	return 1;
//ch6 memory block-----------------------------------------------------------------------------------------
  	ch6 = malloc(sizeof(int32_t) * datacount); /* allocate memory for datacount int's */
 	if (!ch6) { /* If data == 0 after the call to malloc, allocation failed for some reason */
    	perror("Error allocating memory for channel 6");
    	abort();
	}
  	/* ch points to a valid block of memory.
     clearing block. */
  	memset(ch6, 0, sizeof(int32_t)*datacount);

    if (!bcm2835_init())
    	return 1;
//ch7 memory block-----------------------------------------------------------------------------------------
  	ch7 = malloc(sizeof(int32_t) * datacount); /* allocate memory for datacount int's */
 	if (!ch7) { /* If data == 0 after the call to malloc, allocation failed for some reason */
    	perror("Error allocating memory for channel 7");
    	abort();
	}
  	/* ch points to a valid block of memory.
     clearing block. */
  	memset(ch7, 0, sizeof(int32_t)*datacount);
...
//TXT file open--------------------------------------------------------------------------------------------
	...
	FILE *datos3;	
	datos3 = fopen("sen3.txt", "w");
	FILE *datos4;	
	datos4 = fopen("sen4.txt", "w");
	FILE *datos5;	
	datos5 = fopen("sen5.txt", "w");
	FILE *datos6;	
	datos6 = fopen("sen6.txt", "w");
	FILE *datos7;	
	datos7 = fopen("sen7.txt", "w"); 
//SPI setup------------------------------------------------------------------------------------------------
...
//ADS1256 config-------------------------------------------------------------------------------------------
    ...
    printf("Acquiring %ld samples at 3750SPS/channel...\n", datacount);
//LOOP-----------------------------------------------------------------------------------------------------
		while(1){
	    	while((ADS1256_Scan() == 0)) ;
	            ...
	            ch3[size] = ADS1256_GetAdc(ch_num+3) ;
	            ch4[size] = ADS1256_GetAdc(ch_num+4) ;
	            ch5[size] = ADS1256_GetAdc(ch_num+5) ;
	            ch6[size] = ADS1256_GetAdc(ch_num+6) ;
	            ch7[size] = ADS1256_GetAdc(ch_num+7) ;
	            size++ ;
